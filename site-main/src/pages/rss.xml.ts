...
import { getWritingCollection, sortMDByDate } from 'astro-pure/server'

// Get dynamic import of images as a map collection
const imagesGlob = import.meta.glob<{ default: ImageMetadata }>(
  '/src/content/writing/**/*.{jpeg,jpg,png,gif,avif,webp}' // add more image formats if needed
)

const renderContent = async (post: CollectionEntry<'writing'>, site: URL) => {
    ...
    return async function (tree: Root) {
      ...
      visit(tree, 'image', (node) => {
        if (node.url.startsWith('/images')) {
          ...
          const imagePathPrefix = `/src/content/writing/${post.id}/${node.url.replace('./', '')}`
...
const GET = async (context: AstroGlobal) => {
  const allPostsByDate = sortMDByDate(await getWritingCollection()) as CollectionEntry<'writing'>[]
